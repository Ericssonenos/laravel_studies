...
CREATE TABLE IF NOT EXISTS auth.locatarios (
  id_locatario BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  txt_nome_locatario VARCHAR(120) NOT NULL,
  txt_identificador_locatario VARCHAR(80) NOT NULL UNIQUE,
...
COMMENT ON COLUMN auth.locatarios.txt_identificador_locatario IS 'PT: Identificador legível (slug). EN: Readable identifier (slug).';
...
CREATE TABLE IF NOT EXISTS auth.usuarios (
  ...
  txt_nome_usuario VARCHAR(120) NOT NULL,
